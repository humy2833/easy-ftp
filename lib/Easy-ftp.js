...
function getSftp(client, cb){
	if(client.sftp.ftpObj)
	{
		client.sftp.ftpObj.realpath(".", function(err, path){
			if(err)
			{
				delete client.sftp.ftpObj;
				getSftp(client, cb);
			}
			else if(cb) cb(undefined, client.sftp.ftpObj);
		});
	}
	else
	{
		client.sftp(function(err, sftp){
			...
			{
				SftpGetFailCount = 0;
				client.sftp.ftpObj = sftp;
				if(cb) cb(undefined, client.sftp.ftpObj);
			}
		});
	}
}
...
EasyFTP.prototype.ls = function(path, cb){
	...
	else
	{
		...
		if(this.isFTP)
		{
			this.client.list(this.isWindow ? p : " -a " + p, function(err, list){
				var arr = [];
				if(err)
				{
					if(cb) cb(err);
