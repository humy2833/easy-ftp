...
EasyFTP.prototype.init = function(config){
	this.isWindow = this.isPasv = this.isLoginFail = false;
	this.waitCount = 0;
	this.config = xtend(defaultConfig, config);
	this.config.user = this.config.username;
	this.config.pass = this.config.password;
...
EasyFTP.prototype.connect = function(config){
	...
	{	
		this.isPasv = this.isFTP = true;
		if(!this.config.pasvTimeout) this.config.pasvTimeout = 1000;
		...
		self.pwd(function(err, path){
			...
			if(self.isFTP)
			{
				self.client.list("/", function(err, list){
					...
					{
						self.isPasv = false;
						self.jsClient = new JSFTPClient(self.config);
					}
					self.isConnect = true;
					self.emit("open", self.client);
				});
				self.client.system(function(err, os){
					if(!err && os && os.toLowerCase().indexOf("window") > -1) self.isWindow = true;
				});
			}
			else 
			{
...
EasyFTP.prototype.ls = function(path, cb){
	...
	else
	{
		...
		if(this.isFTP)
		{
			if(!this.isPasv)
			{
				this.jsClient.ls(p, ftpBody);
			}
			else
			{
				this.client.list(this.isWindow ? p : " -a " + p, ftpBody);
			}
			function ftpBody(err, list){
				if(err)
				{
					if(cb) cb(err);
				}
				else
				{
					...
					{
						if((list[i].type === 'd' || list[i].type === 1) && (list[i].name === "." || list[i].name === "..")) continue;
						list[i].type = list[i].type === '-' || list[i].type === 0 ? 'f' : 'd';
						if(!list[i].date && list[i].time) list[i].date = new Date(list[i].time);
						arr.push(list[i]);
					}
					if(cb)cb(err, arr);
				}				
