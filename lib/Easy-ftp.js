 ...
 * 현재경로 반환
 */
EasyFTP.prototype.pwd = function(cb, isFirst, errCnt){
	var self = this;
			...
			if(isFirst === true)
			{
				self.client.sftp(function(err, sftp){
					if(err)
					{
						if(sftp)sftp.end();
						if(errCnt === 0) cb(err);
						else self.pwd(cb, isFirst, errCnt === undefined ? ERROR_CNT : --errCnt);
					}
					else
					{
 ...
 * 폴더 이동
 */
EasyFTP.prototype.cd = function(path, cb, errCnt){
	var self = this;
	if(!this.isConnect)
	{
		this.waitConnect(function(){
		...
		else
		{
			self.client.sftp(function(err, sftp){
				if(err)
				{
					if(sftp)sftp.end();
					if(errCnt === 0) cb(err);
					else self.cd(path, cb, errCnt === undefined ? ERROR_CNT : --errCnt);
				}
				else
				{
		...
		else
		{
			self.client.sftp(function(err, sftp){
				if(err)
				{
					if(sftp)sftp.end();
					if(errCnt === 0) cb(err);
					else self.ls(path, cb, errCnt === undefined ? ERROR_CNT : --errCnt);
				}
				else
				{
 ...
 * 파일, 폴더 이동
 */
EasyFTP.prototype.mv = function(oldPath, newPath, cb, errCnt){
	var self = this;
	if(!this.isConnect)
	{
		this.waitConnect(function(){
			...
			*/
			...
			self.client.sftp(function(err, sftp){
				if(err)
				{
					if(sftp)sftp.end();
					if(errCnt === 0) cb(err);
					else self.mv(oldPath, newPath, cb, errCnt === undefined ? ERROR_CNT : --errCnt);
				}
				else
				{
